...
class QQ:
    ...
    def getshuoshuo(self):
        ...
        self.requests.get(self.urlQzone, headers=headers)
        url='http://user.qzone.qq.com/'+self.qq
        r=self.requests.get(url,headers=headers)
        soup = BeautifulSoup(r.text,'html5lib')
        li=soup.find_all('li',class_='f-single f-s-s')
        for neirong in li:
            name=neirong.find('a',class_='f-name q_namecard ').get_text()
            if name==None:
                continue
            liulan=neirong.find('a',class_='state qz_feed_plugin')
            if liulan==None:
                continue
            print('发表用户为：',name) 
            print(liulan.get_text(),'   ',neirong.find('a',class_='item qz_like_btn_v3').get_text())
            print('----------------------------------')
            if neirong.find('div',class_='f-info').get_text()!='':
               print(neirong.find('div',class_='f-info').get_text())
            else:
                print('说说内容为图片或者视频，无法显示')
            zhuanfa=neirong.find('div',class_='txt-box')
            try:
              if zhuanfa:
                 if zhuanfa.find('a'):
                    print('转发自他人：',end='')
                    print(neirong.find('div',class_='txt-box ').get_text().strip(' '))
              print('')
            except:
              print('')
        r=self.requests.get('http://user.qzone.qq.com/3052183420', headers=headers)
        r=self.requests.get('http://user.qzone.qq.com/3052183420', headers=headers)
        print(r.text)

if __name__ == '__main__':
    qq = QQ('***********user**************', '****************passward***********')
    #qq.feed('我就是我，颜色不一样的烟火!')#发说说函数
    qq.getshuoshuo()
